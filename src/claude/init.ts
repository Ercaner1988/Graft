...
/** `.mcp.json`'s write result, plus `skipped` — the one place `--no-mcp`
 * reports the row explicitly rather than omitting it (registerMcpConfigs and
 * claudeGlobalTargets do the latter for every other host). */
type ClaudeMcpWrite = McpWrite | { id: string; path: string; action: 'skipped' };
...
export function runInit(
  ...
  const mcp = opts.mcp === false
    ...
    : mergeJsonKey('claude', mcpTarget, 'mcpServers', serverEntry({ runner: opts.runner, cwd: dir }));
  ...
  const global = opts.global === false
    ? []
    : installClaudeGlobal(opts.home ?? homedir(), { mcp: opts.mcp, hooks: opts.hooks, runner: opts.runner });
